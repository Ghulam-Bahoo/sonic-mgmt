...
from tests.conftest import ptfhost
...
def add_dut_ip(duthost, intfs, ips, prefix_len):
    ...
    for idx in range(len(intfs)):
        cmd_buffer += 'sudo config interface ip add {} {}/{} ;'.format(
            intfs[idx], ips[idx], prefix_len)
        if idx % 50 == 0:
...
def remove_dut_ip(duthost, intfs, ips, prefix_len):
    cmd_buffer = ""
    for idx in range(len(intfs)):
        cmd_buffer += 'sudo config interface ip remove {} {}/{} ;'.format(
            intfs[idx], ips[idx], prefix_len)
        if idx % 50 == 0:
            duthost.shell(cmd_buffer)
            cmd_buffer = ""
...
def get_neighbors_scale(duthost, tbinfo, ipv6=False, scale_count=1):
    ...
    for idx in range(1, scale_count):
        ...
        if ipv6:
            ...
            neighbor_addrs.append(
                t1_ipv4_pattern.format(
                    idx2, rolloveridx * 2 + 1))
            neighbor_devs.append(neighbor_intfs[index])
            ptf_devs.append(ptf_intfs[index])
...
def add_ipaddr(
    ...
    for idx in range(len(neighbor_addrs)):
        if ipv6:
            cmd_buffer += "ip -6 addr add {}/{} dev eth{} ;".format(
                neighbor_addrs[idx], prefix_len, neighbor_interfaces[idx])
        else:
            cmd_buffer += "ip addr add {}/{} dev eth{} ;".format(
                neighbor_addrs[idx], prefix_len, neighbor_interfaces[idx])
        if idx % 50 == 0:
...
def del_ipaddr(
    ...
    for idx in range(len(neighbor_addrs)):
        if ipv6:
            cmd_buffer += "ip -6 addr del {}/{} dev eth{} ;".format(
                neighbor_addrs[idx], prefix_len, neighbor_interfaces[idx])
        else:
            cmd_buffer += "ip addr del {}/{} dev eth{} ;".format(
                neighbor_addrs[idx], prefix_len, neighbor_interfaces[idx])
        if idx % 50 == 0:
...
def check_ptf_bfd_status(ptfhost, neighbor_addr, local_addr, expected_state):
    bfd_state = ptfhost.shell(
        "bfdd-control status local {} remote {}" .format(
            neighbor_addr, local_addr))["stdout"].split("\n")
...
def check_dut_bfd_status(
    ...
    for i in range(max_attempts + 1):
        bfd_state = duthost.shell(
            "sonic-db-cli STATE_DB HGET 'BFD_SESSION_TABLE|default|default|{}' 'state'" .format(neighbor_addr),
            module_ignore_errors=False)['stdout_lines']
        logger.info(
            "BFD state check: {} - {}".format(neighbor_addr, bfd_state[0]))

        if expected_state in bfd_state[0]:
            return  # Success, no need to retry

        logger.error(
            "BFD state check failed: {} - {}".format(neighbor_addr, bfd_state[0]))
        if i < max_attempts:
            time.sleep(retry_interval)

    # If all attempts fail, raise an assertion error
...
def create_bfd_sessions(
    ...
    if scale_test:
        ...
        # Doing so that we can send batches of 50 commands to PTF and keep the
        # code readable.
    ...
    for idx, neighbor_addr in enumerate(neighbor_addrs):
        ...
        ptf_buffer += "bfdd-control connect local {} remote {} ; ".format(
            neighbor_addr, local_addrs[idx])
    ...
    result = duthost.shell(
        'docker exec -i swss swssconfig /dev/stdin < {}'.format(bfd_file_dir),
        module_ignore_errors=True)
    if result['rc'] != 0:
        pytest.fail(
            'Failed to apply BFD session configuration file: {}'.format(
                result['stderr']))
    if dut_init_first:
        ptfhost.shell(ptf_buffer)


def create_bfd_sessions_multihop(
        ptfhost,
        duthost,
        loopback_addr,
        ptf_intf,
    ...
    for neighbor_addr in neighbor_addrs:
        logger.info(
            "create BFD sessions, loopback {} neighbor ip {}".format(
                loopback_addr, neighbor_addr))
        bfd_config.append({
            "BFD_SESSION_TABLE:default:default:{}".format(neighbor_addr): {
...
def bfd_echo_mode(duthost,neighbor_addrs):
    ...
    cmd = "sudo vtysh -c 'configure terminal' -c 'bfd' -c 'bfd echo-mode file {}' -c 'end'".format(
        bfd_echo_config_file)
    duthost.shell(cmd)
...
def update_bfd_session_state(ptfhost, neighbor_addr, local_addr, state):
    ptfhost.shell(
        "bfdd-control session local {} remote {} state {}".format(neighbor_addr, local_addr, state))


def update_bfd_state(ptfhost, neighbor_addr, local_addr, state):
    ptfhost.shell(
        "bfdd-control session local {} remote {} {}".format(neighbor_addr, local_addr, state))
...
def verify_bfd_queue_counters(duthost, dut_intf):
    ...
    for queue_val in range(0, 7):
        queue_pkt_count, _ = get_egress_queue_count(
            duthost, dut_intf, int(queue_val))
        logger.debug(
            "Interface {}, Queue {}, counter {}".format(
                dut_intf, queue_val, queue_pkt_count))
        if queue_pkt_count != 0:
            pytest.fail(
                'Queue {} count is not zero, BFD packets might use this'.format(queue_val))

    bfd_queue = 7
    queue_pkt_count, _ = get_egress_queue_count(
        duthost, dut_intf, int(bfd_queue))
    logger.debug("Queue counters: {}".format(queue_pkt_count))
    if queue_pkt_count == 0:
        pytest.fail('Queue 7 packet count is zero, no BFD traffic')


@pytest.mark.parametrize('dut_init_first',
                         [True, False], ids=['dut_init_first', 'ptf_init_first'])
...
def test_bfd_basic(
    ...
    local_addrs, prefix_len, neighbor_addrs, neighbor_devs, neighbor_interfaces = get_neighbors(
        duthost, tbinfo, ipv6, count=bfd_session_cnt)
    try:
        ...
        for idx, neighbor_addr in enumerate(neighbor_addrs):
            if idx == update_idx:
                ...
                check_ptf_bfd_status(
                    ptfhost, neighbor_addr, local_addrs[idx], "Init")
            else:
                check_dut_bfd_status(duthost, neighbor_addr, "Up")
                check_ptf_bfd_status(
                    ptfhost, neighbor_addr, local_addrs[idx], "Up")
...
def test_bfd_multihop(request, rand_selected_dut, ptfhost, tbinfo,
    ...
    bfd_session_cnt = int(request.config.getoption('--num_sessions'))
    loopback_addr, ptf_intf, nexthop_ip, neighbor_addrs, dut_intf = get_neighbors_multihop(
        duthost, tbinfo, ipv6, count=bfd_session_cnt)
    try:
        cmd_buffer = ""
        for neighbor in neighbor_addrs:
            cmd_buffer += 'sudo ip route add {} via {} ;'.format(
                neighbor, nexthop_ip)
        duthost.shell(cmd_buffer, module_ignore_errors=True)

        create_bfd_sessions_multihop(
            ptfhost,
            duthost,
            loopback_addr,
            ptf_intf,
        ...
        cmd_buffer = ""
        for neighbor in neighbor_addrs:
            cmd_buffer += 'sudo ip route delete {} via {} ;'.format(
                neighbor, nexthop_ip)
        duthost.shell(cmd_buffer, module_ignore_errors=True)
...
def test_bfd_echo_mode(request, rand_selected_dut, ptfhost, tbinfo, ipv6):
    ...
    try:
        ...
        result = duthost.shell(
            "vtysh -c 'show bfd peer'")['stdout'].split('\n')
        error = False
        for line in result:
        ...
        duthost.shell(
            "vtysh -c 'configure terminal' -c 'no bfd' -c 'end' -c 'do write' -c 'exit'")
